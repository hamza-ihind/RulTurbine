        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "sns.histplot(lifespans, bins=20, kde=True, color='#70C9FF', edgecolor='#2E2C7C', alpha=0.7, \n",
                    "             line_kws={'color': '#1B2A5E', 'linewidth': 3})\n",
                    "\n",
                    ...
                    "\n",
                    "plt.axvline(mean_life, color='#2E2C7C', linestyle='--', linewidth=1.5, label=f'Mean: {mean_life:.1f} cycles')\n",
                    "plt.axvline(median_life, color='#3CA4F0', linestyle='-.', linewidth=1.5, label=f'Median: {median_life:.1f} cycles')\n",
                    "\n",
                    "plt.title('Distribution of Engine Lifespans in Training Set (FD001)', fontsize=12, fontweight='bold', pad=15, color='#1B2A5E')\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    plt.plot(unit_data['cycle'], unit_data[sensor_col], color='#4A7CDE', alpha=0.85, linewidth=2)\n",
                    "    \n",
                    "    # Highlight flat/invariant sensors with a light ice-blue background\n",
                    "    if sensor_col in DROPPED_SENSORS:\n",
                    "        plt.gca().set_facecolor('#DCEFFA')\n",
                    ...
                    "                     color='#2E2C7C', fontweight='bold', fontsize=11)\n",
                    "    \n",
                    "    plt.title(f\"Sensor {i}: {sensor_name}\", fontsize=12, fontweight='bold', pad=15, color='#1B2A5E')\n",
                    "    plt.xlabel('Operating Cycles', fontsize=10, color='#1B2A5E')\n",
                    "    unit_label = desc.split('(')[-1].replace(')', '') if '(' in desc else ''\n",
                    "    plt.ylabel(unit_label, fontsize=10, color='#1B2A5E')\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "brand_cmap = LinearSegmentedColormap.from_list('brand_light_blue', ['#DCEFFA', '#70C9FF', '#3CA4F0'])\n",
                    "\n",
                    "sns.heatmap(corr_kept, annot=True, fmt=\".2f\", cmap=brand_cmap, vmin=-1, vmax=1, \n",
                    "            linewidths=0.5, cbar_kws={\"shrink\": .8}, \n",
                    "            annot_kws={\"size\": 9, \"color\": \"#1B2A5E\"})\n",
                    "\n",
                    "plt.title('Correlation Heatmap of the 14 Kept Sensors (FD001)', fontsize=12, fontweight='bold', pad=15, color='#1B2A5E')\n",
                    "plt.tight_layout()\n",
