        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "comparison_df.round(2).style.highlight_min(subset=['Test RMSE', 'PHM08 Score'], color='#DCEFFA')"
                ]
            },
            {
                "cell_type": "markdown",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "        \n",
                    "    # Plotting using brand colors: RF (#6B78B0), XGBoost (#4A7CDE), LSTM (#3CA4F0)\n",
                    "    plt.figure(figsize=(9, 4.5), dpi=300)\n",
                    "    plt.plot(range(1, total_cycles + 1), true_ruls, color='black', linewidth=2.5, label='True Piecewise RUL')\n",
                    "    plt.plot(cycles_plot, rf_preds_unit, color='#6B78B0', linestyle=':', linewidth=2, label='Random Forest')\n",
                    "    plt.plot(cycles_plot, xgb_preds_unit, color='#4A7CDE', linestyle='-.', linewidth=2, label='XGBoost')\n",
                    "    if lstm_preds_unit is not None:\n",
                    "        plt.plot(cycles_plot, lstm_preds_unit, color='#3CA4F0', linestyle='-', linewidth=2, label='LSTM Model')\n",
                    "        \n",
                    "    plt.title(f\"RUL Prediction Trajectory for Test Engine Unit {unit}\", fontsize=12, fontweight='bold', pad=15, color='#1B2A5E')\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "n, bins, patches = plt.hist(errors, bins=25, color='#AFAED7', edgecolor='#1B2A5E', alpha=0.8)\n",
                    "\n",
                    ...
                    "    if right_side < 0:\n",
                    "        patch.set_facecolor('#6B78B0') # Early (safe)\n",
                    "    else:\n",
                    "        patch.set_facecolor('#4A7CDE') # Late (critical)\n",
                    "        \n",
                    "from matplotlib.patches import Patch\n",
                    "legend_elements = [\n",
                    "    Patch(facecolor='#6B78B0', edgecolor='#1B2A5E', label='Early prediction (d < 0, Low Penalty)'),\n",
                    "    Patch(facecolor='#4A7CDE', edgecolor='#1B2A5E', label='Late prediction (d > 0, High Penalty)')\n",
                    "]\n",
                    "\n",
                    "plt.axvline(0, color='#1B2A5E', linestyle='-', linewidth=1.5)\n",
                    "plt.xlabel(\"Prediction Error (d = Predicted RUL - True RUL)\")\n",
                    "plt.ylabel(\"Frequency\")\n",
                    "plt.title(\"LSTM Prediction Error Distribution & Penalty Regions\", fontsize=12, fontweight='bold', pad=15, color='#1B2A5E')\n",
